<script lang="ts">
  ...
  onMount(async () => {
    ...
    node.append('title').text(''); // Remove the title element to prevent the default tooltip
    ...
    node.on('mouseover', function (event, d) {
      ...
      d3.select('body')
        ...
        .attr(
          'style',
          'position: absolute; background: rgba(0,0,0,0.7); color: white; padding: 5px; border-radius: 5px;'
        )
        .html(`${d.name}`)
  ...
  function handleClick(result) {
    nodes.forEach((node) => {
      if (result === node.name) {
        ...
        const width = window.innerWidth;
        const height = window.innerHeight;
        const zoomLevel = d3.select('svg > g').node().getCTM().a;
        console.log(zoomLevel);

        // Center the node: move the target node to the center of the SVG
        const translateX = -node.x * zoomLevel;
        const translateY = -node.y * zoomLevel;
        console.log('Moved to ' + translateX + ' ' + translateY);

        showSearch = false;
        searchQuery = '';

        // Apply the translation to the graph group (<g>)
        d3.select('svg > g')
          .transition()
          .duration(500)
